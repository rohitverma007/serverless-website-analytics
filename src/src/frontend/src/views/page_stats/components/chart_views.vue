...
let period: Ref<"hour" | "day"> = ref("day");
...
async function loadData()
{
  ...
  const filledInData = fillMissingDates(resp, props.fromDate.toISOString(), props.toDate.toISOString(),
    props.sites, period.value === "hour");
  chartViews.value = filledInData;
}
watch(() => [props.sites, props.fromDate, props.toDate, props.filter, period.value], async () => {
  await loadData();
}, {
  deep: true
})
...
function changePeriod(command: "hour" | "day")
{
  period.value = command;
  console.log("changePeriod", command)
  loadData();
}

onMounted(() => {
  watch([isDark, chartViews, chartType, period], () => {
    paintChart();
  });
...
<template>
  <div class="container" style="height: 470px;">
    <div v-show="!loading" style="position: relative; padding: 10px;">
      ...
      <el-dropdown class="chart_selector" trigger="click" @command="changePeriod" style="right: 100px;">
      <span>
        {{period}}
        <el-icon style="top: 2px;"> <arrow-down /> </el-icon>
      </span>
